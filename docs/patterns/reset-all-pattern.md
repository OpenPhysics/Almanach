...
This pattern covers the **Reset All button** wiring every model `Property` back to its `initialValue`. For structuring state so a custom save/load feature can serialize and restore it, see [State Persistence and Save/Restore Patterns](/patterns/state-persistence-and-save-restore-patterns).
:::
